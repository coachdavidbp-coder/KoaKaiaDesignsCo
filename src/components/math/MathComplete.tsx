...
const SCORE_MSGS = [
  { min: 100, emoji: "🌟", msg: "PERFECT SCORE!", sub: "Every problem right — you're a math champion!" },
  { min: 80,  emoji: "⭐", msg: "Math Superstar!", sub: "Amazing math skills! You should be so proud!" },
  { min: 60,  emoji: "👍", msg: "Good Math!", sub: "You're getting stronger every mission. Keep going!" },
  { min: 0,   emoji: "🧮", msg: "Keep Practicing!", sub: "Math takes practice — you're building your brain!" },
];

export function MathComplete({ mission, results, score, xpEarned, onGoHome, onPlayAgain }: MathCompleteProps) {
  ...
  return (
    <div className="min-h-full flex flex-col items-center justify-center px-6 py-10 text-center">
      ...
      >
        <RewardBox emoji="🧮" value={`${correct}/${results.length}`} label="Correct" color={mission.theme.primary} />
        <RewardBox emoji="💎" value={`+${mission.crystalReward}`} label="Crystals" color="#F59E0B" />
      ...
      >
        {Array.from({ length: 5 }).map((_, i) => (
          ...
          >
            <Star className={`w-6 h-6 ${i < starCount ? "text-amber-400 fill-amber-400" : "text-gray-700 fill-gray-800"}`} />
          </motion.div>
        ))}
      </motion.div>
      ...
      >
        <button
          ...
          style={{ background: `linear-gradient(135deg, ${mission.theme.primary}, ${mission.theme.secondary})` }}
        >
